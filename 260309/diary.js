...
// 수정 모드 추적 변수 (제공됨)
// 이면 새 일기 추가 모드, 숫자면 해당 id의 일기를 수정 중
let editingId = null;
...
function renderStats() {
  ...
  totalEl.className = "stat-item stat-total";
  ...
  for (const mood in stats.moods) {
    ...
    moodEl.textContent = `${MOOD_EMOJIS[mood]} ${mood}: ${stats.moods[mood]}개`;
    grid.appendChild(moodEl);
  }
